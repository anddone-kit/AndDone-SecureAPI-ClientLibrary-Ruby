  ...
  class VendorResponseDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::VendorResponseDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::VendorResponseDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, payment_based_id, legal_entity_type, merchant_id, vendor_name, vendor_alias_name, vendor_db_name, payment_method_type, notification_type, vendor_notes, phone_number, is_mobile_number, email, url, vendor_status, use_same_as_physical_address, remittance_address, physical_address, verification_results, created_by, modified_by, created_on, modified_on, approved_date, template].hash
    end
